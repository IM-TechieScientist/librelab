...
static char const setup_page[] =
    ...
    "<title>LibreLab Pico Setup</title></head><body><h1>LibreLab Pico Wi-Fi Setup</h1>"
    "<p>Enter the Wi-Fi network that LibreLab Pico should join.</p>"
...
static esp_err_t configure_handler(httpd_req_t *request)
{
    ...
    httpd_resp_set_type(request, "text/html");
    httpd_resp_sendstr(request, "<h1>Saved</h1><p>LibreLab Pico is joining the selected Wi-Fi network.</p>");
...
bool wifi_provisioning_run(wifi_provisioning_credentials_t *credentials)
{
    ...
    snprintf(ap_ssid, sizeof(ap_ssid), "LibreLab-Pico-%02X%02X", mac[4], mac[5]);
