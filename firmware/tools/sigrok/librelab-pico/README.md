# LibreLab Pico libsigrok Driver

This directory is an out-of-tree `libsigrok` hardware driver for the LibreLab
...
1. Copy this directory to `libsigrok/src/hardware/librelab-pico`.
2. Add the directory to libsigrok's hardware-driver build files.
3. Add `librelab_pico_driver_info` to the libsigrok hardware driver registry if
...
```sh
sigrok-cli --driver librelab-pico:conn=/dev/ttyACM0 --samples 1024
```

PulseView should then show `librelab-pico` as a sigrok hardware driver. Select the
