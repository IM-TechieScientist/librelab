...
{
    uint32_t bits_per_word = librelab_pico_bits_per_word(pin_count);
    if (bits_per_word == 0) {
        return 0;
    }
...
{
    uint32_t bits_per_word = librelab_pico_bits_per_word(pin_count);
    uint32_t word_count = librelab_pico_word_count(pin_count, sample_count);

    if (!dst || !src || pin_count == 0 || pin_count > LIBRELAB_PICO_MAX_CHANNELS ||
...
static int scpi_write(LibreLabPicoDevice *dev, char const *fmt, ...)
{
    char line[128];
    va_list ap;
...
static int read_line(LibreLabPicoDevice *dev, char *line, size_t line_len)
{
    size_t pos = 0;
    ...
    while (pos + 1 < line_len) {
        uint8_t byte;
        int ret = read_byte_timeout(dev->fd, &byte, LIBRELAB_PICO_IO_TIMEOUT_MS);
        if (ret < 0) {
            return ret;
        }
        if (byte == '\n') {
...
int librelab_pico_probe(LibreLabPicoDevice *dev, char *idn, size_t idn_len)
{
    ...
    return strstr(idn, "FOSSASIA") && strstr(idn, "LibreLab Pico") ? 0 : -ENODEV;
}

static int read_scpi_block(LibreLabPicoDevice *dev, uint8_t **payload, size_t *len)
{
    uint8_t header;
    int ret;

    do {
        ret = read_byte_timeout(dev->fd, &header, LIBRELAB_PICO_IO_TIMEOUT_MS);
        if (ret < 0) {
            return ret;
        }
    ...
    uint8_t digits_ch;
    ret = read_byte_timeout(dev->fd, &digits_ch, LIBRELAB_PICO_IO_TIMEOUT_MS);
    if (ret < 0) {
        return ret;
    }
    ...
    if (!end || *end != '\0' || payload_len == 0 ||
        payload_len > LIBRELAB_PICO_MAX_BLOCK_BYTES) {
        return -EPROTO;
    }
    ...
    ret = read_exact_timeout(dev->fd, buf, payload_len, LIBRELAB_PICO_IO_TIMEOUT_MS);
    if (ret < 0) {
        free(buf);
        return ret;
    }
...
static uint32_t divider_for(LibreLabPicoDevice const *dev)
{
    uint32_t samplerate = dev->samplerate_hz ? dev->samplerate_hz
                                             : LIBRELAB_PICO_DEFAULT_SAMPLERATE;
    uint32_t sysclk = dev->sysclk_hz ? dev->sysclk_hz
                                     : LIBRELAB_PICO_DEFAULT_SYSCLK_HZ;
    uint32_t divider = sysclk / samplerate;

    return divider == 0 ? 1 : divider;
}

static int configure_trigger(LibreLabPicoDevice *dev)
{
    int ret;

    switch (dev->trigger) {
    case LIBRELAB_PICO_TRIGGER_HIGH:
    ...
    case LIBRELAB_PICO_TRIGGER_LOW:
        ...
        ret = scpi_write(dev, "LA:CONFigure:TRIGger:PIN %u", dev->trigger_pin);
        return ret < 0 ? ret : scpi_write(dev, "LA:CONFigure:TRIGger:MODE LEVEL");
    case LIBRELAB_PICO_TRIGGER_RISING:
        ret = scpi_write(dev, "LA:CONFigure:TRIGger:LEVel 1");
        if (ret < 0) {
            return ret;
        }
    ...
    case LIBRELAB_PICO_TRIGGER_FALLING:
        ret = scpi_write(dev, "LA:CONFigure:TRIGger:LEVel 0");
        if (ret < 0) {
            return ret;
        }
        ret = scpi_write(dev, "LA:CONFigure:TRIGger:PIN %u", dev->trigger_pin);
        return ret < 0 ? ret : scpi_write(dev, "LA:CONFigure:TRIGger:MODE EDGE");
    case LIBRELAB_PICO_TRIGGER_AUTO:
    default:
...
int librelab_pico_capture(LibreLabPicoDevice *dev, uint8_t **samples, size_t *len)
{
    ...
    if (!dev || !samples || !len || dev->pin_count == 0 ||
        dev->pin_count > LIBRELAB_PICO_MAX_CHANNELS ||
        dev->sample_count == 0 || dev->sample_count > LIBRELAB_PICO_MAX_SAMPLES) {
        return -EINVAL;
    }
    ...
    size_t word_count = librelab_pico_word_count(dev->pin_count, dev->sample_count);
    ...
    bool ok = librelab_pico_unpack_words(
        unpacked,
        dev->sample_count,
        words,
        word_count,
