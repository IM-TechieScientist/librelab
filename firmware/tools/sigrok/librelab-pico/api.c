/*
 * Out-of-tree libsigrok hardware driver for LibreLab Pico.
 *
 * Drop this directory into libsigrok/src/hardware/librelab-pico and register the
...
#define LOG_PREFIX "librelab-pico"
...
static LibreLabPicoDevice *dev_context_new(char const *conn)
{
    LibreLabPicoDevice *devc = g_malloc0(sizeof(*devc));

    devc->fd = -1;
    devc->conn = g_strdup(conn);
    devc->sysclk_hz = LIBRELAB_PICO_DEFAULT_SYSCLK_HZ;
    devc->samplerate_hz = LIBRELAB_PICO_DEFAULT_SAMPLERATE;
    devc->sample_count = LIBRELAB_PICO_DEFAULT_SAMPLES;
    devc->pin_base = LIBRELAB_PICO_DEFAULT_PIN_BASE;
    devc->pin_count = LIBRELAB_PICO_DEFAULT_PIN_COUNT;
    devc->trigger_pin = LIBRELAB_PICO_DEFAULT_PIN_BASE;
    devc->trigger = LIBRELAB_PICO_TRIGGER_AUTO;
    return devc;
}

static void dev_context_clear(LibreLabPicoDevice *devc)
{
    if (!devc) {
        return;
    }

    librelab_pico_close(devc);
    g_free(devc->conn);
    devc->conn = NULL;
}
...
static struct sr_dev_inst *device_new(char const *conn, char const *idn)
{
    ...
    sdi->model = g_strdup("LibreLab Pico");
...
static GSList *scan(struct sr_dev_driver *di, GSList *options)
{
    ...
    if (!conn) {
        sr_info("librelab-pico needs conn=/dev/ttyACM* for scanning.");
        return NULL;
    }
    ...
    LibreLabPicoDevice probe = {
        .fd = -1,
        .conn = (char *)conn,
    };
    char idn[128];

    if (librelab_pico_open(&probe) < 0) {
        sr_warn("Failed to open %s.", conn);
        return NULL;
    }

    int ret = librelab_pico_probe(&probe, idn, sizeof(idn));
    librelab_pico_close(&probe);
    if (ret < 0) {
        sr_warn("%s is not a LibreLab Pico SCPI device.", conn);
        return NULL;
    }
...
{
    LibreLabPicoDevice *devc;

    if (!sdi || !(devc = sdi->priv)) {
        return SR_ERR_ARG;
    }

    if (librelab_pico_open(devc) < 0) {
        return SR_ERR;
    }
...
static uint32_t enabled_channel_count(struct sr_dev_inst const *sdi)
{
    ...
    if (count > LIBRELAB_PICO_MAX_CHANNELS) {
        count = LIBRELAB_PICO_MAX_CHANNELS;
    }
    return count;
}
...
{
    LibreLabPicoDevice *devc = sdi->priv;
    ...
    case SR_TRIGGER_ZERO:
        devc->trigger = LIBRELAB_PICO_TRIGGER_LOW;
        break;
    case SR_TRIGGER_ONE:
        devc->trigger = LIBRELAB_PICO_TRIGGER_HIGH;
        break;
    case SR_TRIGGER_RISING:
        devc->trigger = LIBRELAB_PICO_TRIGGER_RISING;
        break;
    case SR_TRIGGER_FALLING:
        devc->trigger = LIBRELAB_PICO_TRIGGER_FALLING;
        break;
    default:
        devc->trigger = LIBRELAB_PICO_TRIGGER_AUTO;
        break;
...
)
{
    ...
    case SR_CONF_LIMIT_SAMPLES: {
        ...
        if (sample_count == 0 || sample_count > LIBRELAB_PICO_MAX_SAMPLES) {
            return SR_ERR_ARG;
        }
...
)
{
    ...
    case SR_CONF_SAMPLERATE:
        *data = std_gvar_samplerates(samplerates, G_N_ELEMENTS(samplerates));
        break;
    case SR_CONF_LIMIT_SAMPLES:
        *data = std_gvar_tuple_u64(1, LIBRELAB_PICO_MAX_SAMPLES);
        break;
...
static int dev_acquisition_start(const struct sr_dev_inst *sdi)
{
    ...
    int ret = librelab_pico_capture(devc, &samples, &len);
    if (ret < 0) {
        sr_err("LibreLab Pico capture failed: %s.", g_strerror(-ret));
...
static struct sr_dev_driver librelab_pico_driver_info = {
    .name = "librelab-pico",
    .longname = "LibreLab Pico SCPI logic analyser",
    .api_version = 1,
    .init = std_init,
    .cleanup = std_cleanup,
    .scan = scan,
...
SR_REGISTER_DEV_DRIVER(librelab_pico_driver_info);
