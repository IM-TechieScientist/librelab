...
enum {
    LIBRELAB_PAYLOAD_MAGIC = 0x424c424cu, /* "LBLB" little-endian */
    LIBRELAB_PAYLOAD_VERSION = 1,
    LIBRELAB_SUBTYPE_META = 1,
    LIBRELAB_SUBTYPE_DATA = 2,
    LIBRELAB_PAYLOAD_HEADER_LEN = 32,
    LIBRELAB_DATA_BYTES_PER_FRAME =
        ESP_SPI_BRIDGE_PAYLOAD_LEN - LIBRELAB_PAYLOAD_HEADER_LEN,
    LIBRELAB_FORMAT_LA_U32_PACKED = 1,
    LIBRELAB_FORMAT_DSO_U16_LE = 2,
...
)
{
    ...
    put_u32_le(&payload[0], LIBRELAB_PAYLOAD_MAGIC);
    payload[4] = LIBRELAB_PAYLOAD_VERSION;
...
)
{
    ...
    prepare_payload_header(
        payload,
        instrument,
        LIBRELAB_SUBTYPE_META,
        capture_sequence,
        0,
        0,
        24
    );
    put_u32_le(&payload[LIBRELAB_PAYLOAD_HEADER_LEN + 0], sample_rate_for_meta(instrument, meta));
    put_u32_le(&payload[LIBRELAB_PAYLOAD_HEADER_LEN + 4], meta->sample_count);
    put_u32_le(&payload[LIBRELAB_PAYLOAD_HEADER_LEN + 8], meta->channel_count);
    put_u32_le(&payload[LIBRELAB_PAYLOAD_HEADER_LEN + 12], meta->pin_base_or_channel);
    put_u32_le(&payload[LIBRELAB_PAYLOAD_HEADER_LEN + 16], meta->trigger_mode);
    put_u32_le(
        &payload[LIBRELAB_PAYLOAD_HEADER_LEN + 20],
        meta->data_format != 0 ? meta->data_format :
            (instrument == TRANSPORT_INSTRUMENT_DSO ||
             instrument == TRANSPORT_INSTRUMENT_MSO_ANALOG
                 ? LIBRELAB_FORMAT_DSO_U16_LE
                 : LIBRELAB_FORMAT_LA_U32_PACKED)
    );
    ...
    uint16_t chunk_count = (uint16_t)((len + LIBRELAB_DATA_BYTES_PER_FRAME - 1u) /
                                      LIBRELAB_DATA_BYTES_PER_FRAME);
    if (chunk_count == 0) {
        chunk_count = 1;
    }
    ...
    for (uint16_t chunk = 0; chunk < chunk_count; ++chunk) {
        size_t offset = (size_t)chunk * LIBRELAB_DATA_BYTES_PER_FRAME;
        size_t remaining = len > offset ? len - offset : 0;
        size_t chunk_len =
            remaining > LIBRELAB_DATA_BYTES_PER_FRAME ? LIBRELAB_DATA_BYTES_PER_FRAME
                                                   : remaining;

        prepare_payload_header(
            payload,
            instrument,
            LIBRELAB_SUBTYPE_DATA,
            capture_sequence,
            chunk,
            chunk_count,
            (uint16_t)chunk_len
        ...
        if (chunk_len > 0) {
            memcpy(&payload[LIBRELAB_PAYLOAD_HEADER_LEN], &data[offset], chunk_len);
        }
